...
def process_dataset(json_file, output_file):
    ...
    with open(output_file, "w") as f:
        for patient_id, patient_data in dataset.items():
            ...
            print(f"Generated description for patient {patient_id}: {response.choices[0].message.content}")
            # exit()
            # Store the result in the dictionary
            results[patient_id] = response.choices[0].message.content
...
if __name__ == "__main__":
    # Path to the dataset
    json_file = r"E:\acmm_mpdd\Testing\labels\personalized_test.json"
    # Output description file
    output_file = r"E:\acmm_mpdd\Testing\labels/personalized_descriptions_test.json"
